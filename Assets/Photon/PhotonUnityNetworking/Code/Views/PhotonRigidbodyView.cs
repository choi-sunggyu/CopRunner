...
namespace Photon.Pun
{
    ...
    public class PhotonRigidbodyView : MonoBehaviourPun, IPunObservable
    {
        ...
        [HideInInspector] public bool  m_SynchronizeAngularVelocity      = false;
        [HideInInspector] public bool  m_TeleportEnabled                 = false;
        [HideInInspector] public float m_TeleportIfDistanceGreaterThan   = 3.0f;
        ...
        public void FixedUpdate()
        {
            ...
            {
                m_Body.position = Vector3.MoveTowards(
                    m_Body.position,
                    m_NetworkPosition,
                    m_Distance * (1.0f / PhotonNetwork.SerializationRate)
                );

                m_Body.rotation = Quaternion.RotateTowards(
                    m_Body.rotation,
                    m_NetworkRotation,
                    m_Angle * (1.0f / PhotonNetwork.SerializationRate)
                );
            }
        }
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                ...
                {
                    stream.SendNext(m_Body.velocity); // ✅ 수정
                }
            ...
            else
            {
                ...
                {
                    if (Vector3.Distance(m_Body.position, m_NetworkPosition)
                ...
                if (m_SynchronizeVelocity || m_SynchronizeAngularVelocity)
                {
                    ...
                    if (m_SynchronizeVelocity)
                    {
                        m_Body.velocity = (Vector3) stream.ReceiveNext(); // ✅ 수정

                        m_NetworkPosition += m_Body.velocity * lag;       // ✅ 수정

                        m_Distance = Vector3.Distance(
                            m_Body.position,
                            m_NetworkPosition
                        );
                    }
                    ...
                    if (m_SynchronizeAngularVelocity)
                    {
                        ...
                        m_NetworkRotation = Quaternion.Euler(
                            m_Body.angularVelocity * lag
                        ) * m_NetworkRotation;

                        m_Angle = Quaternion.Angle(
                            m_Body.rotation,
                            m_NetworkRotation
                        );
                    }
