...
#pragma warning restore 0109
        ...
        public void Update()
        {
            if (!photonView.AmOwner || !controllable) return;

            if (photonView.CreatorActorNr != PhotonNetwork.LocalPlayer.ActorNumber) return;
        ...
        {
            if (!photonView.IsMine || !controllable) return;
        ...
        {
            rigidbody.velocity        = Vector3.zero; // ✅ 수정
            ...
            if (photonView.IsMine)
            {
                ...
                if (PhotonNetwork.LocalPlayer.CustomProperties
                    .TryGetValue(AsteroidsGame.PLAYER_LIVES, out lives))
                {
